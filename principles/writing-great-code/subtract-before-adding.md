...
First ask what can be removed.

Good subtraction:

- deletes dead branches
- removes one-caller wrappers
- collapses duplicated state
- retires legacy API paths after callers migrate
- narrows scope before adding a mechanism
- removes scattered feature checks by moving ownership to the canonical layer
- reframes state so repeated conditionals disappear

Do not add a layer just to make the current edit feel organized.
Moving complexity to another file is not subtraction.
